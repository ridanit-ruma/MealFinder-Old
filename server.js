...
const sendAlarmSignal = async () => {
    console.log('[INFO] Sending alarm signal...');
    const response = await fetch(`${process.env.ALARM_HOST}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            message: 'send meal info to user'
        })
    });
}

const job = new cron.CronJob('0 13 * * *', updateMealData, null, true, 'Asia/Seoul');
const alarmjob = new cron.CronJob('0 7 * * *', sendAlarmSignal, null, true, 'Asia/Seoul');
job.start();
alarmjob.start();
